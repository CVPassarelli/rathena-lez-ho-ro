...
Mutate only with explicit authority. In the local runtime, only setup may initialize absent secrets; status/logs/smoke/stop/start must assert without generation, and rotation is explicit with a verified backup. Positive checks prove current-cycle inter-server readiness, not merely PID/ports; negative checks cover missing dependency/config and safe failure. Regression covers build, isolated schema, all servers, functional behavior, IDs and exact client. Never discard conflicts, auto-migrate/deploy or call liveness operational readiness. Deliver revisions/range, commands, conflicts/files, security findings, results/failures, untested/client blockers, risks and rollback.
